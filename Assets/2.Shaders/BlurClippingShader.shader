Shader "Custom/BlurClippingShader"
{
    Properties
    {
        ...
        _ClippingEdgeBlur("Edge Blur", Float) = 0.1
    }
        ...
        SubShader
        {
            ...
            Pass
            {
                ...
                float alpha = smoothstep(_ClippingEdgeBlur, 0.0, distanceToClipEdge);

                // 클리핑 영역 내에서는 텍스처 그대로 사용
                fixed4 texColor = tex2D(_MainTexure, i.uv);

                // 투명도 조정 (클리핑 영역 바깥일 경우 투명도 감소)
                texColor.a *= alpha;
